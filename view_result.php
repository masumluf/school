...
$date_exam=$_POST['num'];
if($date_exam==""){
		echo '<p class="result_out"><b>Please Select a Specific Date for Viewing Result.</b></p>';
		exit;
}

  $i=0;
           $class_result = mysql_query("SELECT * FROM class_test_result WHERE `student_id`='$check' && `date`='$date_exam';");  
while($result_row=mysql_fetch_array($class_result)){
 ...
 if($i==0){
	   	echo '<p class="result_out"><b>Nothing Found.</b></p>';
	   }
?>